...
def compute_centroid(rawImage, cameraProps, result_ID):
	...
	# Filter the 2D image
	rawImage = np.divide(rawImage, DEFINES.PC_CAMERA_MAX_INTENSITY_RAW).astype(np.float64)
	...
	rowIn = np.array([np.mgrid[0:crop_size_row_max-crop_size_row_min],]*(crop_size_col_max-crop_size_col_min), dtype=np.float64).transpose()
	colIn = np.array([np.mgrid[0:crop_size_col_max-crop_size_col_min],]*(crop_size_row_max-crop_size_row_min), dtype=np.float64)
...
def _main():
	...
	cameraProps.yCorr = np.zeros((imgSize, imgSize), dtype=np.float64)
	cameraProps.xCorr = np.zeros((imgSize, imgSize), dtype=np.float64)
	cameraProps.ROIoffsetX = 0
	cameraProps.ROIoffsetY = 0
	cameraProps.scaleFactor = 0.033

	xCoordinates = np.array([np.mgrid[0:imgSize],]*(imgSize), dtype=np.float64).transpose()
	yCoordinates = np.array([np.mgrid[0:imgSize],]*(imgSize), dtype=np.float64)

	image = np.zeros((imgSize,imgSize), dtype=np.float64)
	image[0][0] = 255
