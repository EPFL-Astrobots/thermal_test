...
class Positioner(object):
    ...
    def send_all_start_trajectory(self):
        """sends start trajectory to brodcast, id must be 0
    ...
    def get_all_ids(self):
        """requests all ids from bus, id must be 0

        return: list of positioner id
        """
        self._send_command('GET_ID')
    ...
    def goto_absolute(self, alpha, beta):
        ...
        if answer is not None:
            ...
            if result == 'COMMAND_ACCEPTED':
                ...
                log_positioner.error(f'Error with command goto absolute: {result}')
                alpha_time = 0
                beta_time = 0
    ...
    def is_moving(self):
        self.get_status()
        if self.status.displacement_completed == 1:
            return False
        if self.status.displacement_completed == 0:
            return True

    def reset_all_positions(self):
        self._send_command('RESET_ALL_POSITIONS')
        answer = self._get_answer()
        if answer is not None:
            result = self._get_response_code(answer.arbitration_id)
            log_positioner.debug(result)
        else:
            log_positioner.error('Error with command RESET_ALL_POSITIONS')
            return None
        return None
    
    def set_status(self, newFlags):
        """sets the status flags

        :param newFlags: flags to be set high
        """
        packetdata = bytes(struct.pack('<II', newFlags.asInt & 0xFFFFFFFF, 0))
        self._send_command('SET_STATUS_LOW', packetdata)
        ...
        if answer is not None:
            ...
            log_positioner.error(f'Error with command set status low {result}')
            return None
        
        packetdata = bytes(struct.pack('<II', newFlags.asInt >> 32, 0))
        self._send_command('SET_STATUS_HIGH', packetdata)
        ...
        if answer is not None:
            ...
            log_positioner.error(f'Error with command set status high {result}')
            return None
